...
class Com_GamesInstallerScript
{
	...
	public function install($adapter)
	{
		...
		$db->setQuery("UPDATE `#__extensions` SET enabled = 1 WHERE name = 'System - JGames' OR name = 'Content - JGames'");
		$result = $db->query();
							...
							if (jsonResponse.success == true) {
								...
								if (systemMessage) {
									var dt = new Element('dt', {
										'class': 'error',
										'html': 'Error'
									}).inject(systemMessage, 'bottom');
									var dd = new Element('dd', {
										'class': 'error message',
										'html': '<ul><li>' + jsonResponse.msg + '</li></ul>'
									}).inject(systemMessage, 'bottom');
								} else {
									var error = new Element('dl', {
										'id': 'system-message'
									});
				...
				<?php if (count($status->libraries)) : ?>
					...
					<tr class="row<?php echo (++ $rows % 2); ?>">
						<tr class="row0">
							...
							<td><strong><?php echo ($library['result'])?JText::_('Installed'):'<span style="color:red;">'.JText::_('Not installed').'</span>'; ?></strong></td>
						</tr>
					</tr>
					<?php endforeach; ?>
				<?php endif; ?>
