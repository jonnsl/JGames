...
j,width:f.width,height:f.height})).inject(h);(new Element("div",{"class":"clr"})).inject(i);(new Element("input",{type:"hidden",id:"jform_boxart_path_"+b,value:d,name:"jform[boxarts]["+b+"]"})).inject(i);f=(new Element("a",{"class":"modal hasTip",href:"index.php?option=com_media&view=images&tmpl=component&fieldid=jform_boxart_path_"+b+"&folder=",rel:"{handler: 'iframe', size: {x: 800, y: 500}}",html:"Add Boxart"})).inject(i);f.store("tip:title","Add Boxart");f.store("tip:text","Add Boxart for "+e);
new Tips(f);SqueezeBox.assign(f,{parse:"rel"});i.inject("boxarts_inner","top");$("boxarts").getParent("div").setStyle("height","100%");this.Carousel.cacheElements();this.Carousel.toFirst();this.boxarts.include(b)},removeBoxart:function(b){$("boxart_"+b).dissolve({duration:1E3,onComplete:function(b){b.destroy();this.centralizeArrows();this.Carousel.cacheElements()}.bind(this)});this.boxarts.erase(b)}});
